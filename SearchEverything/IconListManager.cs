...
namespace Etier.IconHelper
{
	...
	public class IconListManager
	{
        ...
        public Image GetImage(int index, IconReader.IconSize size)
        {
            Image img;
            try
            {
                if (ManageBothSizes)
                {
                    if (size == IconReader.IconSize.Small)
                    {
                        img = (Image)((ImageList)_imageLists[0]).Images[index];
                    }
                    else // Large
                    {
                        img = (Image)((ImageList)_imageLists[1]).Images[index];
                    }
                }
                else
                {
        ...
        {
            return GetImage(index, IconReader.IconSize.Small);
        }

        public Image GetImage(string filePath) {
            return GetImage(filePath, IconReader.IconSize.Small);
        }
        ...
        public Image GetImage(string filePath, IconReader.IconSize size)
        {
            ...
            {
                iconPos = (int) _extensionList[extension];
            }
            else
            {
                iconPos = 0;
